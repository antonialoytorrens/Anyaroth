...
FrameComponent::FrameComponent(GameObject * obj) : RenderComponent(obj), SpriteComponent(obj) {}
...
void FrameComponent::render(Camera* c) const
{
	...
	SDL_Point anchor = { int(_transform->getAnchor().getX() * winRect.w), int(_transform->getAnchor().getY() * winRect.h) };
