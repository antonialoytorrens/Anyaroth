...
void DebugDraw::DrawSolidPolygon(const b2Vec2* vertices, int32 /*vertexCount*/, const b2Color& /*color*/)
{
	...
	SDL_Rect winRect = { int(poly.x * GAME_RESOLUTION_X / _camera->getCameraSize().getX()), int(poly.y * GAME_RESOLUTION_Y / _camera->getCameraSize().getY()),
		int(poly.w * GAME_RESOLUTION_X / _camera->getCameraSize().getX() + 1), int(poly.h * GAME_RESOLUTION_Y / _camera->getCameraSize().getY() + 1) }; //+1 para el tema del Zoom
